    ...
    try:
        ...
        try:
            ...
            for logo_path, logo_position, opacity in zip(logo_paths, logo_positions, opacities or [1.0]*len(logo_paths)):
                ...
                elif 'disoi.png' in logo_path:
                    target_logo_area = img_area * 0.035  # 3.5% for disoi.png
                elif 'AI.png' in logo_path:
                    target_logo_area = img_area * 0.025  # 2.5% for AI.png
                else:
                    target_logo_area = img_area * 0.035  # Default to 3.5% for any other logos
...
async def ask_for_logo(bot, chat_id, group_id, include_back=False):
    keyboard = [
        ...
        [InlineKeyboardButton("Logo Kenh14.vn", callback_data=f"logo_kenh14_{group_id}")],
        [InlineKeyboardButton("Logo 'Ảnh tạo bởi AI'", callback_data=f"logo_ai_{group_id}")]  # Thêm dòng này
    ]
    if include_back:
...
async def handle_logo_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Xử lý khi chọn logo (disoi hoặc kenh14)
    if action == 'logo':
        ...
        context.user_data['media_groups'][group_id]['logo_display'] = (
            ...
            "Logo 'Ảnh tạo bởi AI'"  # Thêm trường hợp này
        )
...
def main():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    logo_files = ['kenh14.png', 'disoi.png', 'AI.png']  # Thêm AI.png
